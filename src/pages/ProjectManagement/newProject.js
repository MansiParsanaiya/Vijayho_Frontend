    ...
    const formattedData = projects.map((project, index) => {
        ...
        return {
            ...
            viewTask: <> <Link to={`/task/${project._id}`} >
                <Button color="warning" size="sm" >View Tasks</Button>
            </Link></>,
            addTask: <> <Button color="warning" size="sm" onClick={() => { handleOpenTaskModal(index); }}>Add Task</Button></>,
            edit: <> <Button color="warning" size="sm" onClick={() => { setModal(true); handleUpdateStudent(index); }} disabled={isDisabled}>Edit</Button></>,
