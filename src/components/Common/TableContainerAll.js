...
const TableContainerAll = ({
  ...
  const onChangeInSelect = event => {
    ...
    searchValue(searchWord, customPageSizeValue, 1, defaultProject, defaultStatus);
  };

  const onChangeInSelectProject = event => {
    setdefaultProject(String(event.target.value));
    searchValue(searchWord, customPageSizeValue, 1, defaultProject, defaultStatus);
  };

  const onChangeInSelectStatus = event => {
    setdefaultStatus(String(event.target.value));
    searchValue(searchWord, customPageSizeValue, 1, defaultProject, defaultStatus);
  };
  ...
  const onChangeInInput = event => {
    ...
    if (inputValue >= 1 && inputValue <= pageOptionFromParents?.totalPages) {
      searchValue(searchWord, customPageSizeValue, inputValue, defaultProject, defaultStatus);
    }
  };
  ...
  const nextPageMoveDouble = () => {
    searchValue(searchWord, customPageSizeValue, pageOptionFromParents?.totalPages, defaultProject, defaultStatus);
  }
  ...
  const nextPageMove = () => {
    searchValue(searchWord, customPageSizeValue, pageOptionFromParents?.nextPage, defaultProject, defaultStatus);
  }

  const previousPageMove = () => {
    searchValue(searchWord, customPageSizeValue, pageOptionFromParents?.prevPage, defaultProject, defaultStatus);
  }


  useEffect(() => {
    if (typeof searchValue === 'function') {
      searchValue(searchWord, customPageSizeValue, pageState, defaultProject, defaultStatus);
    }
  }, [searchWord, customPageSizeValue, pageState,defaultProject, defaultStatus]);
        ...
        {isProjectFilter && (
          <Col md={3}>
            <select
              ...
              onChange={onChangeInSelectProject}
            >
         ...
         {isStatusFilter && (
          <Col md={2}>
            <select
              className="form-select"
              value={defaultStatus}
              onChange={onChangeInSelectStatus}
            >
              {['Select Status','Completed','Failed','In Progress','In Review','Pending',''].map(statusName => (
...
TableContainerAll.propTypes = {
  preGlobalFilteredRows: PropTypes.any,
};

export default TableContainerAll;
